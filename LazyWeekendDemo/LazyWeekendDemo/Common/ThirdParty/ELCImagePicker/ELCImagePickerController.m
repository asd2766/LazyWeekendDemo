...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    if (!shouldSelect) {
        NSString *msg = [NSString stringWithFormat:@"最多可以选择 %ld 张照片!", (long)self.totalImageCount];
        [self makeToast:msg];
        return NO;
    ...
    if ([[asset.asset valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypePhoto]) {
        NSURL *url = asset.asset.defaultRepresentation.url;
        
        if(_selectedImageArray){
            for (int i = 0; i<_selectedImageArray.count; i++) {
                NSString *uploadimageStr = _selectedImageArray[i];
                if ([uploadimageStr isEqualToString:[NSString stringWithFormat:@"%@", url]]) {
                    [self makeToast:@"你已选择相同的照片，不能重复上传哦"];
                    return NO;
                }
            }
        }
...
//                NSLog(@"size1: %@", data.bytes);
                
                
//                img = [CommonUtil fixOrientation:img];
//                data = UIImageJPEGRepresentation(img, 1);
////                NSLog(@"size2: %@", data);
//                
//                UIImage *img2 = [UIImage imageWithCGImage:imgRef
//                                                   scale:1.0f
//                                             orientation:orientation];
                data = UIImageJPEGRepresentation(img, 1);
//                NSLog(@"size3: %@", data);
