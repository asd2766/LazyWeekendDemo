...
static const CGFloat CSToastVerticalPadding     = 20.0;
...
//    移除之前添加过的toast
    UIView *view = [keywindow viewWithTag:TOAST_TAG];
    if (view != nil) {
        [view removeFromSuperview];
    }
    
    [self performSelector:@selector(delayShowToast:) withObject:message afterDelay:0.6f];
    
}

- (void)delayShowToast:(id)msg{
...
//    [self performSelector:@selector(judeViewController) withObject:nil afterDelay:0.3f];
    [self showToast:toast duration:interval position:position];
}

//-(void)judeViewController{
//     [self showToast:toast duration:interval position:position];
//}
...
//    UIWindow *keywindow = [UIApplication sharedApplication].keyWindow;
    UIWindow *keywindow=[[UIApplication sharedApplication].windows lastObject];
//    [self performSelector:@selector(judeViewController) withObject:nil afterDelay:0.5f];
//    [NSThread sleepForTimeInterval:0.3f];
    
    if (![keywindow isKindOfClass:[UIViewController class]]) {
        keywindow = [[UIApplication sharedApplication].windows objectAtIndex:[UIApplication sharedApplication].windows.count-1];
    }
    //添加现在需要的toast
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    ...
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            return CGPointMake(keywindow.bounds.size.width/2, (toast.frame.size.height / 2) + CSToastVerticalPadding);
        } else if([point caseInsensitiveCompare:@"bottom"] == NSOrderedSame) {
            return CGPointMake(keywindow.bounds.size.width/2, (keywindow.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding);
        } else if([point caseInsensitiveCompare:@"center"] == NSOrderedSame) {
            return CGPointMake(keywindow.bounds.size.width / 2, keywindow.bounds.size.height / 2);
        }
...
//    wrapperView.layer.borderWidth = 3.0;
    ...
    if (CSToastDisplayShadow) {
        wrapperView.layer.shadowColor = RGB(71, 71, 71).CGColor;
        wrapperView.layer.shadowOpacity = CSToastShadowOpacity;
        wrapperView.layer.shadowRadius = CSToastShadowRadius;
        wrapperView.layer.shadowOffset = CSToastShadowOffset;
    }

    wrapperView.backgroundColor = [RGB(71, 71, 71) colorWithAlphaComponent:CSToastOpacity];
    ...
    if (title != nil) {
        ...
        CGSize maxSizeTitle = CGSizeMake((keywindow.bounds.size.width * CSToastMaxWidth) - imageWidth, keywindow.bounds.size.height * CSToastMaxHeight);
//        CGSize expectedSizeTitle = [title sizeWithFont:titleLabel.font constrainedToSize:maxSizeTitle lineBreakMode:titleLabel.lineBreakMode];
        CGSize expectedSizeTitle = [CommonUtil sizeWithString:title fontSize:CSToastFontSize sizewidth:maxSizeTitle.width sizeheight:maxSizeTitle.height];
    ...
    if (message != nil) {
        ...
        CGSize maxSizeMessage = CGSizeMake((keywindow.bounds.size.width * CSToastMaxWidth) - imageWidth, keywindow.bounds.size.height * CSToastMaxHeight);
//        CGSize expectedSizeMessage = [message sizeWithFont:messageLabel.font constrainedToSize:maxSizeMessage lineBreakMode:messageLabel.lineBreakMode];
        CGSize expectedSizeMessage = [CommonUtil sizeWithString:message fontSize:CSToastFontSize sizewidth:maxSizeMessage.width sizeheight:maxSizeMessage.height];
